...
export function NewsletterForm() {
  ...
  const submit = async (data: NewsletterInput) => { setServerError(""); if (process.env.NEXT_PUBLIC_STATIC_EXPORT === "true") { window.location.assign(`mailto:info@twbproductions.co.za?subject=Newsletter signup&body=${encodeURIComponent(`Please add ${data.email} to the TWB mailing list.`)}`); return; } const response = await fetch("/api/newsletter", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }); if (!response.ok) return setServerError("Unable to subscribe right now."); setDone(true); };
